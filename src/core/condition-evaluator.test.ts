...
describe('ConditionEvaluator', () => {
    ...
    describe('matchesConditions', () => {
        it('should match conditions for objects', () => {
            ...
            const guide = { '[type=A]': 'A' };
            expect(ConditionEvaluator.matchesConditions(item, guide, ['[type=A]'])).toBe(true);
        });

        it('should not match if conditions differ', () => {
            const item = { type: 'A', value: 1 };
            const guide = { '[type=B]': 'B' };
            expect(ConditionEvaluator.matchesConditions(item, guide, ['[type=B]'])).toBe(false);
        });

        it('should self-match conditions for preview mode', () => {
            const item = { '[type=A]': 'A', value: 1 };
            const guide = { '[type=A]': 'A' };
            expect(ConditionEvaluator.matchesConditions(item, guide, ['[type=A]'])).toBe(true);
        });
        ...
        it('should match structured object condition against JSON serialized string', () => {
            ...
            expect(ConditionEvaluator.matchesConditions(item, guide, ['[Assert={"FnEquals":["prod","prod"]}]'])).toBe(
                true
            );
        });
    });
    ...
    describe('matchesSerializedCondition', () => {
        ...
        it('should match consecutive bracket serialized conditions', () => {
            const item = { type: 'A', status: 'active' };
            const serialized = '[type=A][status=active]';
            expect(ConditionEvaluator.matchesSerializedCondition(item, serialized)).toBe(true);
        });
        ...
        it('should match structured object condition accurately and reject different object', () => {
            ...
            expect(
                ConditionEvaluator.matchesConditions(item, matchedGuide, ['[Assert={"FnEquals":["prod","prod"]}]'])
            ).toBe(true);
            // 異なる構造のオブジェクト条件に対する不一致検証
            expect(
                ConditionEvaluator.matchesConditions(item, differentGuide, ['[Assert={"FnEquals":["prod","stg"]}]'])
            ).toBe(false);
        });
